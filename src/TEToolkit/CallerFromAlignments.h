...
public:
    ...
    //Float64HashTable *
    std::map<int,double> pqtable ;                  // remember pvalue->qvalue convertion
    ...
    CallerFromAlignments(ShortRead * treat, ShortRead * ctrl,std::vector<int> ctrl_d_s,
                          std::vector<double> ctrl_scaling_factor_s,
                         int d = 200,
                         double treat_scaling_factor = 1.0,
                         double pseudocount = 1.0,
                         int end_shift = 0,
                         double lambda_bg = 0, bool no_lambda_flag = false,
                         bool save_bedGraph = false,
                         std::string  bedGraph_filename_prefix = "PREFIX",
                         std::string bedGraph_treat_filename = "TREAT.bdg",
                         std::string bedGraph_control_filename = "CTRL.bdg",
                         std::string cutoff_analysis_filename = "TMP.txt",
                         bool save_SPMR = false);
    
    ~CallerFromAlignments();
    
    void call_peaks(PeakIO * peaks, std::vector<std::string> scoring_function_symbols, std::vector<double> score_cutoff_s,  bool uniqOnly, int min_length = 200, int max_gap = 50,  bool call_summits = false, bool auto_cutoff = false);
private:
    std::map<std::string,double> pscore_map;
    ...
    std::vector<double> __cal_pscore ( std::vector<double> array1, std::vector<double> array2 );
    ...
    void __chrom_call_peak_using_certain_criteria (PeakIO  *peaks, std::string chrom, std::vector<std::string> scoring_function_s, std::vector<double> score_cutoff_s, int min_length,int max_gap, bool uniqOnly,bool call_summits, bool save_bedGraph );
