...
void CallerFromAlignments::__pileup_treat_ctrl_a_chromosome ( std::string chrom , bool uniqOnly )
{
    ...
    try{
        ...
        for (size_t i=0 ; i < chr_pos_treat_ctrl.pos.size(); i ++ ) {
            ...
            f  << chr_pos_treat_ctrl.pos[i] << "\t" << tr_v << "\t" << std::setprecision(10) << ct_v << "\n";
        }
        f.close();
    }
...
pos_tc_t CallerFromAlignments::__pileup_treat_ctrl_a_chromosome_multiThread ( std::string chrom , bool uniqOnly )
{
    ...
    //info( "chrom " + chrom + "   res_pos_tc pos size = " + std::to_string(res_pos_tc.pos.size()) );
    // save data to temporary file
    if (uniqOnly) {
...
std::vector<double> CallerFromAlignments::__cal_pscore ( std::vector<double> array1, std::vector<double> array2 ,bool uniqOnly)
{
    ...
    for (size_t i=0 ; i< array1.size();i++)
    {
      if(uniqOnly){
          ...
          //std::cout << "cal_pscore " << array1[i] <<"\t"<< array2[i]<<"\t"<<r << std::endl;
        //s.push_back( get_pscore_v2( int(array1[i]), array2[i] ));
          s.push_back(r);
     }
    }
    return s;
...
void CallerFromAlignments::__cal_pvalue_qvalue_table ()
{
    ...
    for(size_t  i = 0; i < canoChromlist.size() ; i++)
    {
        ...
        for (size_t j=0; j< chr_pos_treat_ctrl.pos.size() ; j++)
        {
            ...
            //std::cout  << chr_pos_treat_ctrl.pos[j] << "\t" << int(chr_pos_treat_ctrl.treat_v[j]) << "\t" << std::fixed << chr_pos_treat_ctrl.ctrl_v[j] << "\t" << std::fixed << roundTop(this_v,10) << "\n";
           //std::cout  << chr_pos_treat_ctrl.pos[j] << "\t" << int(chr_pos_treat_ctrl.treat_v[j]) << "\t" << chr_pos_treat_ctrl.ctrl_v[j] << "\t" << this_v << "\n";
  ...
  /*  try{
        ...
        for(auto key : keys){
            ...
            f << std::setprecision(10) << key << "\t" << pqtable->get_item(key) << "\n";
        /*    if (key > 1.87 && key < 1.88) {
                pqtable->get_item(t);
                std::cout.precision(15);
                
                std::cout << "key = " << std::fixed << t << std::endl;
                std::cout.precision(17);
                
                std::cout << "key = " << std::fixed << t << std::endl;
                //double test = 1.8723;
                //std::cout << "find 1.8723 " << pqtable->get_item(test) << std::endl;
            }*/
...
void CallerFromAlignments::__chromlist_call_peak_using_certain_criteria(std::vector<std::string> chromlist,PeakIO *peaks, call_peak_options_t copts )
{
    for ( auto chrom : chromlist){
        ...
        __chrom_call_peak_using_certain_criteria ( peaks, chrom, copts , save_bedGraph );
    }
}
...
void CallerFromAlignments::merge_peaklist(PeakIO *peaks, std::vector<PeakIO *> peaks_list,bool uniqOnly)
{
   ...
   // else {
    ...
    //        p->set_CandidatePeakFile(peaks->CandidatePeakFile);
    //        p->write_candidate_to_bed();
...
void CallerFromAlignments::call_peaks ( PeakIO *peaks, call_peak_options_t copts )
{
    ...
    // close bedGraph file
 /*   if (save_bedGraph){
        this->bedGraph_treat_f.open(this->bedGraph_treat_filename,std::ofstream::out);
        this->bedGraph_ctrl_f.open(this->bedGraph_control_filename, std::ofstream::out );
        
        this->bedGraph_treat_f << "track type=bedGraph name=\"treatment pileup\" description=\"treatment pileup after possible scaling for \'UTF-8\'" << "\n";
        
        this->bedGraph_ctrl_f << "track type=bedGraph name=\"control lambda\" description=\"control lambda after possible scaling for \'UTF-8\'" << "\n";
...
bool CallerFromAlignments::__close_peak_wo_subpeaks (bool uniqOnly,std::vector<peak_content_t> peak_content, PeakIO *peaks, int min_length, std::string chrom, int smoothlen, std::vector<std::vector<double> > score_array_s, std::vector<double> score_cutoff_s)
{
    ...
    { // # if the peak is too small, reject it
        ...
        //}
       // else {
       //     summit_p_score = get_pscore_v2( int(summit_treat), summit_ctrl );
       // }
            peaks->add( chrom,           // chromosome
                       peak_content[0].s, // start
                       peak_content[peak_content.size()-1].e, // end
...
/*         else {
           summit_p_score = get_pscore_v2( int(summit_treat), summit_ctrl );
           PeakContent pt ;
           pt.start = peak_content[0].s;
           pt.end = peak_content[peak_content.size()-1].e;
           pt.summit = summit_pos;
           pt.score = summit_q_score;
           pt.pileup = summit_treat;     
           pt.pscore = summit_p_score; // pvalue
           pt.fc = 1.0* ( summit_treat + pseudocount ) / (summit_ctrl + pseudocount ); // fold change
           pt.qscore =  summit_q_score; // qvalue
           peaks->append_candidate_to_bed( chrom, pt);          // chromosome
       }*/
...
void CallerFromAlignments::__write_bedGraph_for_a_chromosome(std::string chrom,pos_tc_t chr_pos_treat_ctrl_multiThread)
{
    ...
    for(int i =1; i < l ; i ++)
    {
        ...
        if (std::abs(pre_v_t - v_t) > 1e-5) // precision is 5 digits
        {
            ...
            tmp_bedGraph_treat_f << chrom << "\t" << pre_p_t << "\t" << p << "\t" << pre_v_t << "\n";
            pre_v_t = v_t;
            pre_p_t = p;
        }
        if ( std::abs(pre_v_c - v_c) > 1e-5) // precision is 5 digits
        {
            ...
            tmp_bedGraph_ctrl_f << chrom << "\t" << pre_p_c << "\t" << p << "\t" << pre_v_c << "\n";
            pre_v_c = v_c;
            pre_p_c = p;
        }
...
void CallerFromAlignments::__chrom_call_peak_using_certain_criteria ( PeakIO *peaks, std::string chrom, call_peak_options_t copts,  bool save_bedGraph )
{
   ...
   /* if(!copts.uniqOnly){
          FE_ret = apply_FE_cutoff(chr_pos_treat_ctrl_multiThread.treat_v, chr_pos_treat_ctrl_multiThread.ctrl_v,above_cutoff_index_array );
          above_cutoff_index_array.clear();
         above_cutoff_index_array = FE_ret;
    }*/
