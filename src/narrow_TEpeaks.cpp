...
int cal_max_dup_tags ( int genome_size, int tags_number, double p=1e-5 )
{
...
//int run_narrow_TEpeaks_all(char* cur_dir, char* optfile, char* tfile,char* cfile, double ratio,
//                            int fragsize, bool wig, char* prj_name, double pval,
//                            double qval, bool nomodel,bool toLarge,
//                            int thredNum, int numItr,char* format, char* keepdup,int gsize)

//int run_narrow_TEpeaks_all(char* cur_dir, char* optfile, char* tfile,char* cfile)
...
int run_narrow_TEpeaks_all(opt_t &options)
{
    ...
    //std::string ctrl_filename = options.cfile;
    //options.cfile = "";
    
    //read alignments of both unique reads and multi-reads
    //save unique reads into memory, for multi-reads only count the numbers
    std::pair<ShortRead *, ShortRead *> treat_input_pair = read_aligmentFile(options,BOTH) ; //treatment sample total tag/fragment
    ...
    {
        tag = "fragment";
        options.onauto =false; //paired-end sample don't need to buil peak model to estimate frament length
    }
    ...
    if (options.keepDuplicates != "all") {
        if (options.keepDuplicates == "auto") {
            ...
            info("#2 calculate max duplicate " + tag + "s in single position based on binomial distribution ...");
        ...
        info("#2  total " + tag + "s after filtering in treatment: " + lexical_cast<std::string>(t1));
        info("#2  total " + tag + "s after filtering in control: " + lexical_cast<std::string>(c1));
    }
...
//    }
    ...
    std::string narrowFile = options.data_outdir + "/" + options.project_name +"_uniqpeaks.narrowPeak";
    ...
    //ADD    info("#4 Write summits bed file... " + options.summitbed);
    //FILE *ofhd_summits = fopen( options.summitbed, "w" );
    //ADD    peakdetect.peaks.write_to_summit_bed (options.summitbed, "peak_", options.project_name,
    //ADD                                          "",score_column, options.trackline );
    //fclose(ofhd_summits);
 ...
 //ADD   info("#4 Write output xls file... " + options.peakxls);
    
//ADD
    
    
    //4.2 peaks in BED
//ADD    std::string score_column = "";
 //ADD   if (options.log_pvalue)
 //ADD   {
//ADD        score_column = "pscore";
//ADD    }
//ADD    else {
//ADD        if (options.log_qvalue)
//ADD        {
//ADD            score_column = "qscore";
//ADD        }
//ADD    }
